...
// Type: Game.Objects.SecondaryObjectReferencesSystem
...
{
  [CompilerGenerated]
  public class SecondaryObjectReferencesSystem : GameSystemBase
  {
    private EntityQuery m_ObjectQuery;
    private SecondaryObjectReferencesSystem.TypeHandle __TypeHandle;
    ...
    protected override void OnUpdate()
    {
      ...
      SecondaryObjectReferencesSystem.UpdateSecondaryObjectReferencesJob jobData = new SecondaryObjectReferencesSystem.UpdateSecondaryObjectReferencesJob()
      {
      ...
      // ISSUE: reference to a compiler-generated field
      this.Dependency = jobData.Schedule<SecondaryObjectReferencesSystem.UpdateSecondaryObjectReferencesJob>(this.m_ObjectQuery, this.Dependency);
    }
        ...
        if (chunk.Has<Deleted>(ref this.m_DeletedType))
        {
          for (int index = 0; index < nativeArray2.Length; ++index)
          {
            ...
            if (this.m_SubObjects.TryGetBuffer(nativeArray2[index].m_Owner, out bufferData))
              CollectionUtils.RemoveValue<SubObject>(bufferData, new SubObject(subObject));
          }
        }
        else
        {
          ...
          if (chunk.Has<Temp>(ref this.m_TempType))
          {
            for (int index = 0; index < nativeArray2.Length; ++index)
            {
              ...
              {
                // ISSUE: reference to a compiler-generated field
                CollectionUtils.TryAddUniqueValue<SubObject>(this.m_SubObjects[owner.m_Owner], new SubObject(nativeArray1[index]));
              }
            }
          }
          else
          {
            for (int index = 0; index < nativeArray2.Length; ++index)
            {
              Entity subObject = nativeArray1[index];
              // ISSUE: reference to a compiler-generated field
              CollectionUtils.TryAddUniqueValue<SubObject>(this.m_SubObjects[nativeArray2[index].m_Owner], new SubObject(subObject));
            }
          }
        }
      }
