...
namespace Game.Prefabs
{
  public struct MaintenanceVehicleSelectData
  {
    ...
    public MaintenanceVehicleSelectData(SystemBase system)
    {
      ...
      this.m_ObjectGeometryDataType = system.GetComponentTypeHandle<ObjectGeometryData>(true);
      ...
      out JobHandle jobHandle)
    {
      ...
      this.m_ObjectGeometryDataType.Update(system);
    ...
    public Entity SelectVehicle(
      ref Random random,
      MaintenanceType allMaintenanceTypes,
      MaintenanceType anyMaintenanceTypes,
      float4 maxParkingSizes)
    {
      return this.GetRandomVehicle(ref random, allMaintenanceTypes, anyMaintenanceTypes, maxParkingSizes);
    }
      ...
      bool parked)
    {
      ...
      {
        prefab = this.GetRandomVehicle(ref random, allMaintenanceTypes, anyMaintenanceTypes, maxParkingSizes);
        if (prefab == Entity.Null)
          return Entity.Null;
      }
      ...
      float4 maxParkingSizes)
    {
      ...
      for (int index1 = 0; index1 < this.m_PrefabChunks.Length; ++index1)
      {
        ...
        NativeArray<ObjectGeometryData> nativeArray3 = prefabChunk.GetNativeArray<ObjectGeometryData>(ref this.m_ObjectGeometryDataType);
        ...
        for (int index2 = 0; index2 < nativeArray2.Length; ++index2)
        {
          ...
          if ((maintenanceVehicleData.m_MaintenanceType & allMaintenanceTypes) == allMaintenanceTypes && ((maintenanceVehicleData.m_MaintenanceType & anyMaintenanceTypes) != MaintenanceType.None || anyMaintenanceTypes == MaintenanceType.None) && this.m_RequirementData.CheckRequirements(ref chunk, index2))
          {
            bool4 bool4 = VehicleUtils.GetParkingSize(nativeArray3[index2], out float _).xyxy > maxParkingSizes;
            if (!math.all(bool4 | bool4.yxwz))
            {
